...
/**
 * PPSLoader class directly loads the results of the PPSEnricher
 * into the products/similars index.
 */
class PPSLoader(ctx:RequestContext,params:Map[String,String]) extends BaseLoader(ctx,params) {
  ...
  override def load(params:Map[String,String]) {
    ...
    if (ctx.putSources("products","similars",sources) == false)
      throw new Exception("Loading process has been stopped due to an internal error.")
    
  }

  private def extract(store:String):RDD[ParquetPPS] = {
    ...
    parquetFile.map(row => {
      ...
      val other = data("other").asInstanceOf[Int]
      val score = data("score").asInstanceOf[Double]

      ParquetPPS(site,item,other,score)
      
    })
    
  }
  
  private def transform(params:Map[String,String],dataset:RDD[ParquetPPS]):List[XContentBuilder] = {
