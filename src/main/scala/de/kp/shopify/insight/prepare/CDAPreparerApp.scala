...
object CDAPreparerApp extends PreparerApp("CDAPreparer") {
  ...
  def main(args:Array[String]) {
    ...
    try {
       ...
       */
      val req_params = createParams(args) ++ Map("name" -> "CDA")
      /*
       * Load orders from Elasticsearch order database and 
       * start Preparer actor to extract CDA data from the
       ...
       */
      val actor = system.actorOf(Props(new CDAHandler(ctx,orders)))   
      inbox.watch(actor)
  ...
  class CDAHandler(ctx:RequestContext,orders:RDD[InsightOrder]) extends Actor {
    ...
    override def receive = {
      ...
      case msg:StartPrepare => {
        ...
        println("CDAPreparerApp started at " + start)
 
        val customer = msg.data("customer").toInt
        
        val preparer = context.actorOf(Props(new CDAPreparer(ctx,customer,orders)))          
      ...
      case msg:PrepareFailed => {
        ...
        println("CDAPreparerApp failed at " + end)
      ...
      case msg:PrepareFinished => {
        ...
        val end = new java.util.Date().getTime           
        println("CDAPreparerApp finished at " + end)
