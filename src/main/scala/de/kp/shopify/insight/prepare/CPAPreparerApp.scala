...
object CPAPreparerApp extends PreparerApp("CPAPreparer") {
  ...
  def main(args:Array[String]) {
    ...
    try {
       ...
       */
      val req_params = createParams(args) ++ Map("name" -> "CPA")
      /*
       * Load orders from Elasticsearch order database and 
       * start Preparer actor to extract CPA data from the
       ...
       */
      val actor = system.actorOf(Props(new CPAHandler(ctx,orders)))   
      inbox.watch(actor)
  ...
  class CPAHandler(ctx:RequestContext,orders:RDD[InsightOrder]) extends Actor {
    ...
    override def receive = {
      ...
      case msg:StartPrepare => {
        ...
        println("CPAPreparerApp started at " + start)
 
        val customer = msg.data("customer").toInt
        
        val preparer = context.actorOf(Props(new CPAPreparer(ctx,customer,orders)))          
      ...
      case msg:PrepareFailed => {
        ...
        println("CPAPreparerApp failed at " + end)
      ...
      case msg:PrepareFinished => {
        ...
        val end = new java.util.Date().getTime           
        println("CPAPreparerApp finished at " + end)
