...
object STMPreparerApp extends PreparerApp("STMPreparer") {
  ...
  def main(args:Array[String]) {
    ...
    try {
       ...
       */
      val req_params = createParams(args) ++ Map("name" -> "STM")
      /*
       * Load orders from Elasticsearch order database and 
       * start Preparer actor to extract STM data from the
       ...
       */
      val actor = system.actorOf(Props(new STMHandler(ctx,orders)))   
      inbox.watch(actor)
  ...
  class STMHandler(ctx:RequestContext,orders:RDD[InsightOrder]) extends Actor {
    ...
    override def receive = {
      ...
      case msg:StartPrepare => {
        ...
        println("STMPreparerApp started at " + start)
 
        val customer = msg.data("customer").toInt
        
        val preparer = context.actorOf(Props(new STMPreparer(ctx,customer,orders)))          
      ...
      case msg:PrepareFailed => {
        ...
        println("STMPreparerApp failed at " + end)
      ...
      case msg:PrepareFinished => {
        ...
        val end = new java.util.Date().getTime           
        println("STMPreparerApp finished at " + end)
