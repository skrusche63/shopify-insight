...
object PPFPreparerApp extends PreparerApp("PPFPreparer") {
  ...
  def main(args:Array[String]) {
    ...
    try {
       ...
       */
      val req_params = createParams(args) ++ Map("name" -> "PPF")
      /*
       * Load orders from Elasticsearch order database and 
       * start Preparer actor to extract PPF data from the
       ...
       */
      val actor = system.actorOf(Props(new PPFHandler(ctx,orders)))   
      inbox.watch(actor)
  ...
  class PPFHandler(ctx:RequestContext,orders:RDD[InsightOrder]) extends Actor {
    ...
    override def receive = {
      ...
      case msg:StartPrepare => {
        ...
        println("PPFPreparerApp started at " + start)
 
        val customer = msg.data("customer").toInt
        
        val preparer = context.actorOf(Props(new PPFPreparer(ctx,customer,orders)))          
      ...
      case msg:PrepareFailed => {
        ...
        println("PPFPreparerApp failed at " + end)
      ...
      case msg:PrepareFinished => {
        ...
        val end = new java.util.Date().getTime           
        println("PPFPreparerApp finished at " + end)
