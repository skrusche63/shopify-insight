...
object ASRPreparerApp extends PreparerApp("ASRPreparer") {
  ...
  def main(args:Array[String]) {
    ...
    try {
       ...
       */
      val req_params = createParams(args) ++ Map("name" -> "ASR")
      /*
       * Load orders from Elasticsearch order database and 
       * start Preparer actor to extract ASR data from the
       ...
       */
      val actor = system.actorOf(Props(new ASRHandler(ctx,orders)))   
      inbox.watch(actor)
  ...
  class ASRHandler(ctx:RequestContext,orders:RDD[InsightOrder]) extends Actor {
    ...
    override def receive = {
      ...
      case msg:StartPrepare => {
        ...
        println("ASRPreparerApp started at " + start)
 
        val customer = msg.data("customer").toInt
        
        val preparer = context.actorOf(Props(new ASRPreparer(ctx,customer,orders)))          
      ...
      case msg:PrepareFailed => {
        ...
        println("ASRPreparerApp failed at " + end)
      ...
      case msg:PrepareFinished => {
        ...
        val end = new java.util.Date().getTime           
        println("ASRPreparerApp finished at " + end)
