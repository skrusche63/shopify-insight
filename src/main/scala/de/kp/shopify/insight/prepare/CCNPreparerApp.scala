...
object CCNPreparerApp extends PreparerApp("CCNPreparer") {
  ...
  def main(args:Array[String]) {
    ...
    try {
       ...
       */
      val req_params = createParams(args) ++ Map("name" -> "CCN")
      /*
       * Load orders from Elasticsearch order database and 
       * start Preparer actor to extract CCN data from the
       ...
       */
      val actor = system.actorOf(Props(new CCNHandler(ctx,orders)))   
      inbox.watch(actor)
  ...
  class CCNHandler(ctx:RequestContext,orders:RDD[InsightOrder]) extends Actor {
    ...
    override def receive = {
      ...
      case msg:StartPrepare => {
        ...
        println("CCNPreparerApp started at " + start)
 
        val customer = msg.data("customer").toInt
        
        val preparer = context.actorOf(Props(new CCNPreparer(ctx,customer,orders)))          
      ...
      case msg:PrepareFailed => {
        ...
        println("CCNPreparerApp failed at " + end)
      ...
      case msg:PrepareFinished => {
        ...
        val end = new java.util.Date().getTime           
        println("CCNPreparerApp finished at " + end)
