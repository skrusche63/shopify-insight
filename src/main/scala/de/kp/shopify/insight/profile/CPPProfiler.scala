...
class CPPProfiler(ctx:RequestContext,params:Map[String,String]) extends BaseActor(ctx) {
  ...
  override def receive = {
    ...
    case message:StartProfile => {
      ...
      ctx.listener ! String.format("""[INFO][UID: %s] %s profiling request received at %s.""",uid,name,start)
      ...
      try {
        ...
        ctx.listener ! String.format("""[INFO][UID: %s] %s profiling finished at %s.""",uid,end)

        
      } catch {
        case e:Exception => {

          ctx.listener ! String.format("""[ERROR][UID: %s] %s profiling failed due to an internal error.""",uid,name)
