...
/**
 * The FRQAnalyzer evaluates the RFM table created by the RFMPreparer
 * based on the quantiles computed before and registers the result as
 * a Parquet file
 */
class FRQAnalyzer(requestCtx:RequestContext) extends BaseActor {
  ...
  override def receive = {
    ...
    case msg:StartAnalyze => {
      ...
      try {
         ...
         */
        val store = String.format("""%s/FRQ/%s""",requestCtx.getBase,uid)         
        table.saveAsParquetFile(store)

        val params = Map(Names.REQ_MODEL -> "FRQ") ++ req_params
        ...
        case e:Exception => {
           ...
           */
          requestCtx.listener ! String.format("""[ERROR][UID: %s] FRQ analysis exception: %s.""",uid,e.getMessage)
  ...
  private def evaluate(params:Map[String,String]):RDD[ParquetFRQ] = {
     ...
     */
    val quantiles = sc.broadcast(rawset.take(1)(0)("F_Quantiles").asInstanceOf[String].split(",").map(v => {
    ...
    rawset.map(x => {
      ...
      val total = x("F").asInstanceOf[Int]
      /*
      ...
      val state = (
          if (total < b1) "L"
          else if (b1 <= total && total < b2) "M"
          else if (b2 <= total) "H"
          else "-"  
      )
      
      ParquetFRQ(site,user,total,state)
