...
/**
 * The CLVAnalyzer evaluates the RFM table created by the RFMPreparer
 * based on the quantiles computed before and registers the result as
 * a Parquet file
 */
class CLVAnalyzer(requestCtx:RequestContext) extends BaseActor {
  ...
  override def receive = {
    ...
    case msg:StartAnalyze => {
      ...
      try {
         ...
         */
        val store = String.format("""%s/CLV/%s""",requestCtx.getBase,uid)         
        table.saveAsParquetFile(store)

        val params = Map(Names.REQ_MODEL -> "CLV") ++ req_params
        ...
        case e:Exception => {
           ...
           */
          requestCtx.listener ! String.format("""[ERROR][UID: %s] CLV analysis exception: %s.""",uid,e.getMessage)
  ...
  private def evaluate(params:Map[String,String]):RDD[ParquetCLV] = {
     ...
     */
    val quantiles = sc.broadcast(rawset.take(1)(0)("M_Quantiles").asInstanceOf[String].split(",").map(v => {
    ...
    rawset.map(x => {
      ...
      val amount = x("M").asInstanceOf[Double]
      /*
      ...
      val state = (
          if (amount < b1) "L"
          else if (b1 <= amount && amount < b2) "M"
          else if (b2 <= amount) "H"
          else "-"  
      )
      
      ParquetCLV(site,user,amount,state)
