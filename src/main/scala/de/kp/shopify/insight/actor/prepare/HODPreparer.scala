...
class HODPreparer(requestCtx:RequestContext,orders:RDD[InsightOrder]) extends BaseActor(requestCtx) {
    ...
    case msg:StartPrepare => {
      ...
      try {
        ...
        val table = s0.flatMap(x => {
          ...
          /* Time of day: 0..23 */
          val supp = timestamps.map(x => new DateTime(x).hourOfDay().get).groupBy(x => x).map(x => (x._1,x._2.size))
          val pref = supp.map(v => (v._1, Math.log(1 + v._2.toDouble / total.toDouble)))
          
          supp.map(v => ParquetHOD(
              site,
              user,
              /* hour */
              v._1.toInt,
         ...
         */
        val store = String.format("""%s/HOD/%s""",requestCtx.getBase,uid)         
        table.saveAsParquetFile(store)

        val params = Map(Names.REQ_MODEL -> "HOD") ++ req_params
        ...
        case e:Exception => {
           ...
           */
          requestCtx.listener ! String.format("""[ERROR][UID: %s] HOD preparation exception: %s.""",uid,e.getMessage)
