...
class AggregateQuestor(requestCtx:RequestContext) extends BaseActor(requestCtx) {
    ...
    case query:AggregateQuery => {
      ...
      try {
        ...
        case e:Exception => {
          ...
          requestCtx.listener ! String.format("""[ERROR][UID: %s] Aggregate query failed: %s.""",uid,e.getMessage)
