...
class HSMBuilder(requestCtx:RequestContext) extends BaseActor {
  ...
  override def receive = {
    ...
    case message:StartBuild => {
      ...
      requestCtx.listener ! String.format("""[INFO][UID: %s] Hidden state model building started.""",uid)
      ...
      response.onSuccess {
        ...
        case result => {
          ...
          if (res.status == ResponseStatus.FAILURE) {
            ...
            requestCtx.listener ! String.format("""[ERROR][UID: %s] Hidden state model building failed due to an engine error.""",uid)
      ...
      response.onFailure {
        ...
        case throwable => {
          ...
          requestCtx.listener ! String.format("""[INFO][UID: %s] Hidden state model building finished.""",uid)
    ...
    case event:StatusEvent => {
      ...
      requestCtx.listener ! String.format("""[INFO][UID: %s] Hidden state model building started.""",event.uid)
      /*
