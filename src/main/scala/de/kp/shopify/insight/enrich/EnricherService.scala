...
class EnricherService(val appName:String) extends SparkService {
  
  protected val sc = createCtxLocal("EnrichContext",Configuration.spark)      
  protected val system = ActorSystem("EnrichSystem")
  ...
  private def registerESTask(params:Map[String,String]) = {
    ...
    val key = "ENRICH:" + params(Names.REQ_NAME) + ":" + params(Names.REQ_UID)
    val task = "Enrich purchase data with " + appName + "."
    /*
